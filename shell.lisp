...
(defun run-prog (prog &rest opts &key args (wait t) &allow-other-keys)
  ...
  #+openmcl (apply #'ccl:run-program prog args :wait wait opts)
  #+ecl (si:system (format nil "~a~{ ~a~}" prog args))
...
(defun pipe-output (prog &rest args)
  ...
  #+ecl (si:open-pipe (format nil "~a~{ ~a~}" prog args)) 
  #+sbcl (sb-ext:process-input (sb-ext:run-program prog args :input :stream
                                                   :output t :wait nil))
  #+openmcl (ccl:external-process-input-stream (ccl:run-program prog
								args :input :stream
								:output t :wait
								nil))
  #-(or allegro clisp cmu gcl lispworks lucid sbcl openmcl)
...
(defun pipe-input (prog &rest args)
  ...
  #+sbcl (sb-ext:process-output (sb-ext:run-program prog args :output :stream
                                                    :error t :input t :wait nil))
  #+openmcl (ccl:external-process-output-stream (ccl:run-program
prog args :output :stream
                                                    :error t :input
t :wait nil))
  #-(or allegro clisp cmu gcl lispworks lucid sbcl openmcl)
